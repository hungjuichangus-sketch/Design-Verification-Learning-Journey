...
* *S\_WAIT\_ANS: Waits for valid random number generation.*  
* *S\_WAIT\_D0 \- S\_WAIT\_D3: Input sequence for the 4 digits.*  
* *S\_CHECK: Compares the input registers against the answer registers to calculate A and B scores using combinational logic.*  
* *S\_FAIL / S\_PASS: Updates game status and writes to memory.*
...
2. *Assign pins according to the DE10-Lite User Manual (or import the .qsf file).*  
3. *Compile the project.*  
4. *Program the .sof file to the DE10-Lite board via USB-Blaster.*
